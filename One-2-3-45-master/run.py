...
def predict_multiview(shape_dir, args):
    ...
    # === STEP 1: Preprocess with SAM (then free SAM from memory) ===
    print("Loading SAM for preprocessing...")
    predictor = sam_init(args.gpu_idx)
    input_raw = Image.open(args.img_path)
    input_256 = preprocess(predictor, input_raw)
    
    # Free SAM from memory
    del predictor, input_raw
    gc.collect()
    torch.cuda.empty_cache()
    print("SAM unloaded. Memory freed.")

    # === STEP 2: Load Zero123 and generate multi-view ===
    print("Loading Zero123-XL...")
    models = init_model(device, 'zero123-xl.ckpt', half_precision=args.half_precision, load_safety_checker=False)
    ...
    # Free Zero123 from memory before reconstruction
    del model_zero123, models, input_256
    gc.collect()
    torch.cuda.empty_cache()
    print("Zero123 unloaded. Memory freed.")
